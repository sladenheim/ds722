...
These techniques allow us to infer unknown population parameters based on sample data.
...
We want to fit a normal distribution $N(\mu, \sigma^2)$ to this data. Parameter estimation in this case would be finding the right values for the parameters $\mu$ and $\sigma$.
...
where $\Theta\subset\mathbb{R}^{k}$ is the parameter space and $\theta = (\theta_1,\ldots, \theta_{k})$ is the parameter vector.
...
$$
\begin{align*}
\tau &= P(X>1) = 1 - P(X\leq 1) = 1-P\left( \frac{X-\mu}{\sigma}<\frac{1-\mu}{\sigma}\right) \\
&=  1-P\left( Z<\frac{1-\mu}{\sigma}\right) = 1 - \Phi\left(\frac{1-\mu}{\sigma}\right)
\end{align*}
$$
...
Recall the $k$th moment of a random variable $X$ is

$$
\alpha_{k} = E(X^{k}) = \int x^{k}f(x)dx,
$$

where $f(x)$ is the pdf of the random variable.

::: {.notes}
These estimators are not always optimal but are easy to compute.
:::
...
$$
\alpha_{j}(\theta) = E_{\theta}(X^{j}) = \int x^{j}f(x;\theta)dx.
$$
...
::: {.notes}
Solving this system yields the method of moments estimator.
:::
...
::: {.notes}
Recall that $\alpha_{1} = E(X)=p$ and $\hat{\alpha}_{1}=\frac{1}{n}\sum_{i=1}^{n}X_{i}$. Equating these two values gives
...
::: {.notes}
Recall that $\alpha_{1} = E_{\theta}(X_{1})=\mu$ and  $\alpha_{2} = E_{\theta}(X_{1}^{2})=V_{\theta}(X_{1}) +E_{\theta}(X_{1})^{2} = \sigma^{2} + \mu^{2}$. Then we have the system of equations
...
::: {.notes}
The pmf is $f(x;p) = p^{x}(1-p)^{1-x}$ for $x\in\{0,1\}$. The likelihood is
...
$$
Y\,|\,X=x\;\sim\;N(f(x;\beta_{0},\beta_{1}), \sigma^{2}).
$$
...
$$
p(y\,|\,X=x, \beta_{0},\beta_{1},\sigma^{2}) = \frac{1}{\sqrt{2\pi}\sigma}\exp\!\left(-\frac{(y-(\beta_{0}+\beta_{1}x))^{2}}{2\sigma^{2}}\right)
$$
:::
...
$$
\mathcal{L}_{n}(\beta_{0},\beta_{1}, \sigma^{2}) = \prod_{i=1}^{n}\frac{1}{\sqrt{2\pi}\sigma}\exp\!\left(-\frac{(y_{i}-(\beta_{0}+\beta_{1}x_{i}))^{2}}{2\sigma^{2}}\right)
$$
:::
...
What is the log-likelihood $\ell_{n}(\beta_{0}, \beta_{1},\sigma^{2})$?
...
The term we care about is
$$
\Vert y - X\beta \Vert_{2}^{2},
$$
which we know the gradient is

$$
-2X^{\top}(y-X\beta).
$$
When this is zero we obtain the familiar LSQ solution
...
**Example: Bernoulli**
Let $X_{i}\sim\operatorname{Bernoulli}(p)$. Let $\hat{p}_{n} = \frac{1}{n}\sum_{i=1}^{n}X_{i}$ denote the MLE of $p$. Is $\hat{p}_{n}$ biased or unbiased?

::: {.notes}
$$
\begin{align}
\operatorname{bias}(\hat{p}_{n}) &= E(\hat{p}_{n}) - p \\
&= E\left(\frac{1}{n}\sum_{i=1}^{n}X_{i} \right) - p \\
...
$$
V(\hat{\theta}) = V(g(x^{(1)},x^{(2)},\ldots, x^{(n)})),
$$
...
::: {.notes}
$$V\left(\hat{p}_n\right) = V\left( \frac{1}{n} \sum_{i=1}^n X_{i} \right)
=\frac{1}{n^2}V\left( \sum_{i=1}^n X_{i} \right) $$
...
$$
\small{\operatorname{MSE}\left(\hat{\theta}_n\right) = E\left(\left(\hat{\theta}_n - \theta\right)^2\right)}.
$$

The MSE is a good single number for evaluating an estimator, because it turns out that:
    
$$\small{\operatorname{MSE}\left(\hat{\theta}_n\right) = \operatorname{bias}\left(\hat{\theta}_n\right)^2 + V\left(\hat{\theta}_n\right)}.
$$
...
1. The MLE is asymptotically optimal or efficient. This means that among all well-behaved estimators, the MLE has the smallest variance for large enough samples.
...
for _ in range(num_simulations):
    swipes = rng.binomial(1, true_theta, n)
...
plt.hist(estimated_thetas, bins=30, color='cyan', edgecolor='black')
...
Recall that the pmf of the discrete uniform distribution is
...
$$
E(X) = \sum_{k=1}^{N}kf(k) = \frac{1}{N}\sum_{k=1}^{N}k = \frac{1}{N}\frac{(N+1)N}{2} = \frac{N+1}{2}. 
$$
...
:::: {.fragment}
$$
f(X; N) = \frac{1}{N}, \quad \text{for } N \geq 9.
$$
::::
