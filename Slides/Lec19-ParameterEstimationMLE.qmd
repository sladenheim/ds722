...
&= \mathbb{E}\left(\frac{1}{n}\sum_{i=1}^{n}X_{i} \right) - p \\
&= \frac{1}{n}\sum_{i=1}^{n}\mathbb{E}(X_{i}) - p \\
...
$$
\mathbb{V}(\hat{\theta}) = \mathbb{V}(g(x^{(1)},x^{(2)},\ldots, x^{(n)})),
$$
...
::: {.notes}
$$\operatorname{Var}\left(\hat{p}_n\right) = \mathbb{V}\left( \frac{1}{n} \sum_{i=1}^n X_{i} \right)
...
$$
\small{\operatorname{MSE}\left(\hat{\theta}_n\right) = \mathbb{E}\left(\left(\hat{\theta}_n - \theta\right)^2\right)}.
$$

The MSE is a good single number for evaluating an estimator, because it turns out that:
    
$$\small{\operatorname{MSE}\left(\hat{\theta}_n\right) = \operatorname{bias}\left(\hat{\theta}_n\right)^2 + \operatorname{Var}\left(\hat{\theta}_n\right)}.
$$
