...
The previous set of equations for the vectors $a_{j}$ as

$$
\begin{align*}
q_{1} &= \frac{a_{1}}{r_{11}} \\
q_{2} &= \frac{a_{2}-r_{12}a_{1}}{r_{22}} \\
...
$$
r_{ij} = q_{i}^{\top}a_{j}\quad (i\neq j),
$$

and

$$
\vert r_{jj}\vert = \left\Vert a_{j} - \sum_{i=1}^{j-1}r_{ij}q_{j} \right\Vert_{2}.
$$
...
$$
q_{1} = \frac{P_{1}a_{1}}{\Vert P_1a_{1}\Vert_2}, q_{2} = \frac{P_{2}a_{2}}{\Vert P_2a_{1}\Vert_2}, \ldots, q_{n} = \frac{P_{n}a_{n}}{\Vert P_{n}a_{n}\Vert_2},
$$
...
$$
q_{j} = a_{j} -\sum_{i=1}^{j-1}(q_{i}^{\top}a_{j})q_{i}.
$$
...
Observe that
$$
q_{2} = a_{2} - (q_{1}^{\top}a_{2})q_{1} = (I-q_{}q_{1}^{\top})a_{2}.
$$
...
$$
q_{j} = (I - \sum_{i=1}^{j-1}q_{i}q_{i}^{\top})a_{j}.
$$
