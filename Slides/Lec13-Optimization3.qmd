...
\text{subject to}~&H=H^{\top}~\text{and}~Hy_{k}=s_{k}.
\end{align}
$$
...
**Theorem**
Let $B_{0}$ be any symmetric positive definite initial matrix, and let $x_{0}$ be a starting point for which Assumption 1 is satisfied. Then the sequence $x_{k}$ generated by the BFGS algorithm (with $\varepsilon=0$) converges to the minimizer $x^{*}$ of $f$.
...
**Theorem**
Suppose that $f$ is twice continuously differentiable and that the iterates generated by the BFGS algorithm converged to the minimizer $x^{*}$ at which Assumption 2 holds. Suppose also that $\sum_{k=1}^{\infty}\Vert x_{k}-x^{*}\Vert < \infty$. Then $x_{k}$ converges to $x^{*}$ at a superlinear rate.
