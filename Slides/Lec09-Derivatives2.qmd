...
:::: {.incremental}
- Jacobian of a scalar-valued function $f(\mathbf{x})$ where $x\in\mathbb{R}^{n}$ is a vector $\nabla f(\mathbf{x})\in\mathbb{R}^{n}$.
- Jacobian of a vector-valued function $\mathbf{f}(\mathbf{x})$ where $x\in\mathbb{R}^{n}$ and $\mathbf{f}\in\mathbb{R}^{m}$ is a matrix $J\in\mathbb{R}^{m\times n}$.
