...
# phi(alpha) for a search direction that is a genuine descent direction,
# i.e. phi'(0) < 0.
def phi(a):
    return np.sin(a - 1) + 0.1 * (a - 5)**2

def dphi(a):
    return np.cos(a - 1) + 0.2 * (a - 5)

phi0, dphi0 = phi(0.0), dphi(0.0)
c1, c2 = 0.3, 0.7  # exaggerated for visual clarity; in practice c1 is tiny (e.g. 1e-4)

alpha = np.linspace(0, 10, 1000)
y = phi(alpha)
armijo_line = phi0 + c1 * alpha * dphi0
armijo_ok = y <= armijo_line
curvature_ok = dphi(alpha) >= c2 * dphi0
acceptable = armijo_ok & curvature_ok

fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(alpha, y, lw=2.5, label=r'$\phi(\alpha)$')
ax.plot(alpha, armijo_line, '--', color='C3',
        label=r"Armijo line: $\phi(0)+c_1\alpha\phi'(0)$")
ax.fill_between(alpha, y.min() - 0.3, y.max() + 0.3, where=acceptable,
                 color='green', alpha=0.15, label='satisfies both Wolfe conditions')
ax.plot(0, phi0, 'ko', markersize=6)
...
Any $\alpha$ in the shaded region simultaneously lies below the Armijo line (sufficient decrease) and has a sufficiently flattened slope (curvature condition) -- these are exactly the step lengths the Wolfe conditions accept.
...
Let's use the exact line search step length $\alpha_{k} = \frac{\nabla f_{k}^{\top}\nabla f_{k}}{\nabla f_{k}^{\top}Q\nabla f_{k}}$ derived above to plot the convergence. Since $Q=I$, this simplifies to $\alpha_{k}=1$ for every step -- so we should expect convergence in a single step.
...
plt.title(f'Optimization Trajectory (Exact Line Search): {n_steps} step{"s" if n_steps != 1 else ""} to converge')
plt.xlabel('$x_1$')
plt.ylabel('$x_2$')
plt.legend()
plt.grid(True)
...
for i in range(max_iter):
    ...
    alpha = exact_line_search(Q1, x1, grad1)
    x1 = x1 - alpha * grad1
...
Let's again use the exact line search step length to plot the convergence. Unlike Example 1, $Q\neq I$ here, so $\alpha_{k}$ now changes every iteration -- but convergence should still be much faster than with a small fixed step.
...
plt.clabel(contour, inline=True, fontsize=8)
plt.plot(trajectory[:, 0], trajectory[:, 1], marker='o', color='red', label='Gradient Descent Path')
plt.title(f'Optimization Trajectory (Exact Line Search): {n_steps} steps')
plt.xlabel('$x_1$')
plt.ylabel('$x_2$')
plt.legend()
plt.grid(True)
...
for i in range(max_iter):
    ...
    alpha = exact_line_search(Q2, x2, grad2)
    x2 = x2 - alpha * grad2
...
Computing $\nabla f_{k}=\frac{1}{m}\sum_{i=1}^{m}\nabla f_{i}(w_{k})$ exactly requires touching *every* one of the $m$ training examples before taking a single step -- for large $m$ (millions of examples, as is typical in ML) this is expensive and wasteful, since early steps don't need to be precise.

Stochastic gradient descent (SGD) instead estimates the gradient using a single (or small batch of) randomly sampled example, trading exactness for speed: each update is cheap and noisy, but many cheap noisy steps often make more progress per unit time than a few expensive exact ones.
...
::: {.notes}
Watch out for a notational subtlety if you inspect PyTorch's source: `torch.optim.SGD` with momentum accumulates the velocity as $v_{k+1}=\beta v_{k} + \nabla f_{k}$ (no $\alpha$ inside the recursion) and only scales by the learning rate at the very last step, $w_{k+1}=w_{k}-\alpha v_{k+1}$. This is a different (but closely related) convention from the one above, where $\alpha$ is baked into $v_{k+1}$ itself. The two agree once $\alpha$ is held constant and the dust settles, but they are not literally the same recursion -- if $\alpha$ changes over time, or during the first few "warm-up" iterations, the two conventions can produce slightly different trajectories. Different libraries (and papers) are not always consistent about which one they mean by "momentum," so it is worth checking when comparing implementations.
:::
...
1. $\quad w_{k+1} = w_{k}-\frac{\alpha}{\sqrt{E_{k+1} + \varepsilon}}\nabla f_{k}$ (element-wise square root and division)

---

Note that $w_{k}, \nabla f_{k}, E_{k}\in\mathbb{R}^{n}$ all have the *same* shape -- $E_{k}$ keeps one running average **per coordinate** of $w$, not a single scalar. Every operation below (square, square root, division) is applied **element-wise**, coordinate by coordinate.

Written out per coordinate $i=1,\ldots,n$, step 5 is

$$
\small
(w_{k+1})_{i} = (w_{k})_{i} - \frac{\alpha}{\sqrt{(E_{k+1})_{i} + \varepsilon}}(\nabla f_{k})_{i}.
$$

::: {.notes}
Think of $E_{k+1}$ as an exponentially weighted moving average of squared gradients -- one such average is tracked independently for *each* coordinate of $w$.

Because everything is element-wise, each coordinate gets its own effective step size $\alpha/\sqrt{(E_{k+1})_i+\varepsilon}$: when a coordinate's gradient has consistently small magnitude, its moving average is small, so its denominator is small and that coordinate takes a larger step. When a coordinate's gradient fluctuates or is large, its moving average grows, shrinking that coordinate's step and preventing overshooting or divergence -- independently of what is happening in the other coordinates. This is the key difference from a single global learning rate.
:::
...
# Optimizer: RMSprop
# NOTE on naming: PyTorch's `alpha` argument is the decay/smoothing factor,
# i.e. what our pseudocode calls gamma -- NOT our learning rate alpha.
# The learning rate is PyTorch's `lr`, which matches our pseudocode's alpha.
optimizer = torch.optim.RMSprop(
    [x],
    lr=0.1,        # our alpha (learning rate)
    alpha=0.9,     # our gamma (decay/smoothing for E_k, common: 0.9-0.99)
    eps=1e-8,      # our epsilon
...
::: {.notes}
The $1-\beta^{k+1}$ term corrects for the fact that the moving average started at zero (bias fades over time). For warming up the training. Without this you understimate the true gradient information.
:::
