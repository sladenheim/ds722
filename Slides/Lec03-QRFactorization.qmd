...
If we divide $\mathbf{v}_{j}$ by $\Vert\mathbf{v}_{j}\Vert_2$, then we have an orthonormal $\mathbf{q}_{j}$. If we have repeated this process $j$ times, then $\mathbf{q}_{j}\in\langle\mathbf{a}_{1}\ldots, \mathbf{a}_{j} \rangle$ and by construction is orthonormal to $\mathbf{q}_1,\ldots \mathbf{q}_{j-1}$. 
...
$$
\vert r_{jj}\vert = \left\Vert \mathbf{a}_{j} - \sum_{i=1}^{j-1}r_{ij}\mathbf{q}_{j} \right\Vert_{2}.
$$
...
Compute $r_{13} = \mathbf{q}_{1}^{T}\mathbf{a}_{3}$
...
Compute $r_{23} = \mathbf{q}_{2}^{T}\mathbf{a}_{3}$
