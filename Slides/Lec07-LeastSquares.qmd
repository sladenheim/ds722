...
$$
\hat b = x_{1} + a_{1}x_{2},
$$ 

that is the best fit to the data. 

The hat over $\hat b$ indicates that it's a prediction, not the value $b$ observed in the data.
...
Alternatively, we could have used the fact if $A=\hat{Q}\hat{R}$, then  $P=\hat{Q}\hat{Q}^{\top}$ is the orthogonal projector onto $\operatorname{range}(A)$, thus
