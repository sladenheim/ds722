...
- the transpose of a matrix $Q$ with elements $(Q)_{ij}$ is the matrix $Q^{T}$, where $(Q^{T})_{ij} = (Q)_{ji}$
- a matrix $Q\in\mathbb{R}^{m\times m}$ is symmetric if $Q=Q^{T}$
...
Let $\mathbf{x},\mathbf{y}\in\mathbb{R}^{2}$, how can we picture the inner product $(\mathbf{x},\mathbf{y}) = \mathbf{y}^{T}\mathbf{x}$?
...
plt.quiver(0, 0, x[0], x[1], angles='xy', scale_units='xy', scale=1, color='r', label=r'$\mathbf{x}$')
plt.quiver(0, 0, y[0], y[1], angles='xy', scale_units='xy', scale=1, color='b', label=r'$\mathbf{y}$')
plt.quiver(0, 0, proj_y_on_x[0], proj_y_on_x[1], angles='xy', scale_units='xy', scale=1, color='g',
           label=r'$\mathrm{proj}_{\mathbf{x}}\mathbf{y} = \frac{\mathbf{x}^T\mathbf{y}}{\mathbf{x}^T\mathbf{x}}\mathbf{x}$')
...
The quantity $\mathbf{q}_{j}^T\mathbf{v}$ is a scalar and we can use these as the coefficients of a linear combination of vectors $\mathbf{q}_{i}$ in order to express $\mathbf{v}$.
...
$$
\mathbf{r} = \mathbf{v} - (\mathbf{q}_{1}^T\mathbf{v})\mathbf{q}_{1} - (\mathbf{q}_{2}^T\mathbf{v})\mathbf{q}_{2}.
$$
...
If $\mathbf{v}\in\operatorname{range}(P)$, then $P\mathbf{v}$ lies precisely on its own shadow. Let $\mathbf{v} = P\mathbf{x}$, then
...
Let $\{\mathbf{q}_{1},\ldots,\mathbf{q}_{n} \}$ be a set of  $n$ orthonormal vectors in $\mathbb{R}^{m}$. Recall that any vector 
$\mathbf{v}$ has the decomposition
$$
\mathbf{v} = \mathbf{r} + \sum_{i=1}^{n}(\mathbf{q}_{i}\mathbf{q}_{i}^{*})\mathbf{v},
$$
...
then $QQ^{T}$ is an orthogonal projector onto $\operatorname{range}(Q)$. 

*Any* product of this form is a projector onto the column space of $Q$ as long as its columns are orthonormal.

Note that $I-QQ^{T}$ is also an orthogonal projector and projects onto the space orthogonal to $\operatorname{range}(Q)$.
...
Can we reconstruct $QQ^{T}$ from $P_{q_i}$?
...
# Plot the original vector v
plt.quiver(0, 0, v[0], v[1], angles='xy', scale_units='xy', scale=1, color='r', label=r'$\mathbf{v}$')

# Plot the projections of Pq @ v and (I-Pq) @ v
plt.quiver(0, 0, proj_v_q1[0], proj_v_q1[1], angles='xy', scale_units='xy', scale=1, color='c', label=r'$P_{q_{1}}\mathbf{v}$')
plt.quiver(0, 0, proj_v_perp[0], proj_v_perp[1], angles='xy', scale_units='xy', scale=1, color='m', label=r'$(I-P_{q_{1}})\mathbf{v}$')

# Plot the vector q_1 
plt.quiver(0, 0, q1[0], q1[1], angles='xy', scale_units='xy', scale=1, color='g', label=r'$\mathbf{q}_1$')
...
plt.title(r'Projection of vector $\mathbf{v}$')
plt.xlabel('x')
plt.ylabel('y')
...
## Projection with $\mathbf{a}$

For an arbitrary non-unit vector $\mathbf{a}$, the analogous operators are

$$
P_{a} = \frac{\mathbf{a}\mathbf{a}^{T}}{\mathbf{a}^{T}\mathbf{a}},
$$

and

$$
P_{\perp a} = I-\frac{\mathbf{a}\mathbf{a}^{T}}{\mathbf{a}^{T}\mathbf{a}}. 
$$
...
if $A^{T}Ax=0$ then we must show $x=0$. Consider

$$
0 = x^{T}A^{T}Ax = (Ax)^{T}Ax = \Vert Ax\Vert_{2}^{2}. 
$$
...
# Plot the projection of v onto the column space of A
ax.quiver(0, 0, 0, proj_v[0], proj_v[1], proj_v[2], color='b', label=r'$A(A^{T}A)^{-1}A^{T}\mathbf{v}$')

# Plot the basis vectors of the column space of A
for i, vec in enumerate(basis_vectors):
    ax.quiver(0, 0, 0, vec[0], vec[1], vec[2], color='g', label=fr'$\mathbf{{a}}_{{{i+1}}}$')
