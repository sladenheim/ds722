...
Recall from the previous lecture the optimization problem: 

Given an objective function $f(x)$, compute
...
where $p_{k}=-\nabla f_{k}$ is the gradient of the objective function at the point $x_{k}$, i.e., $\nabla f_{k} = \nabla f(x_{k})$.
...
Let $x_{k} +\alpha_{k} p_{k}$ and $\alpha_{k}$ satisfies the Wolfe conditions. Suppose that $f$ is bounded below in $\mathbb{R}^{n}$ and continuously differentiable in an open set $\mathcal{N}$ containing the level set $\mathcal{L} = \{x : f(x)\leq f(x_{0})\}$. Assume $\nabla f$ is Lipschitz continuous on $\mathcal{N}$, i.e., there exists $L>0$ such that
...
When $Q$ is SPD, the quantity 

$$
\Vert x \Vert_{Q}^{2} = x^{T}Qx,
$$
...
**Theorem**
The steepest descent method with exact line searches is applied to the strongly convex quadratic function $f(x)$, the error norm satisfies

$$
\Vert x_{k+1} - x^{*}\Vert_{Q}^{2} \leq \left(\frac{\lambda_{n} - \lambda_{1}}{\lambda_{n} + \lambda_{1}}\right)^{2} \Vert x_{k+1} - x^{*}\Vert_{Q}^{2},
$$
...
Consider the objective function $f(x) = \frac{1}{2}x^{T}Qx - b^{T}x$ where $x=\begin{bmatrix} x_{1} \\ x_{2}\end{bmatrix}$, $Q=I$, and $b=\begin{bmatrix} 1 \\ 1\end{bmatrix}$.

Let's plot the convergence of this.
...
for _ in range(max_steps):
    g = grad_f(x)
    alpha = np.dot(g, g) / np.dot(g, np.dot(Q, g))  # Exact line search step size
    x = x - alpha * g
    trajectory.append(x.copy())
    n_steps += 1
    if np.linalg.norm(b-x) < 1e-10:
        break
...
plt.title(f'Optimization Trajectory: {n_steps} step to converge')
plt.xlabel('$x_1$')
plt.ylabel('$x_2$')
plt.legend()
plt.grid(True)
...
Consider the objective function $f(x) = \frac{1}{2}x^{T}Qx - b^{T}x$ where $x=\begin{bmatrix} x_{1} \\ x_{2}\end{bmatrix}$, $Q=\begin{bmatrix} 200 & -1 \\ -1 & 1 \end{bmatrix}$, and $b=\begin{bmatrix} 1 \\ 1\end{bmatrix}$. The eigenvalues for this matrix are approximately 100 and 1. The true minimizer is $x^{*} = \begin{bmatrix} \frac{2}{99} \\ \frac{101}{99} \end{bmatrix}$.
...
df1 = pd.DataFrame(results1, columns=["Iteration", "x1", "x2", "Gradient Norm", "Function Value"])
print(df1.head())
```
...
# Create DataFrame and save to CSV
df2 = pd.DataFrame(results2, columns=["Iteration", "x1", "x2", "Gradient Norm", "Function Value"])
print(df2.head(8))
```
...
$$
f(w) = \frac{1}{m}\sum_{i=1}f_{i}(w; x_{i}, y_{i}),
$$
...
Q_np = np.array([[100.0, -1.0], [-1.0, 1.0]])
...
plt.title(f'Optimization Trajectory (Momentum): {n_steps} step(s)')
plt.xlabel('$x_1$')
plt.ylabel('$x_2$')
plt.legend()
plt.grid(True)
...
optimizer = torch.optim.RMSprop(
    [x],
    lr=0.1,       # try 0.005–0.02 on this quadratic
...
plt.scatter([x_min[0].item()], [x_min[1].item()], c='black', s=60, marker='*', label='Optimum')
plt.title(f'Optimization Trajectory (RMSprop): {n_steps} step(s)')
plt.xlabel('$x_1$')
plt.ylabel('$x_2$')
plt.legend()
plt.grid(True)
...
1. $\quad v_{k+1} = \beta_{2} v_{k} + (1-\beta_{1})\nabla f_{k}^{2}$ (element-wise square)
...
# Optimizer: Adam
# Note: default lr=1e-3 is very stable; you can try 0.005–0.02 on this quadratic.
optimizer = torch.optim.Adam(
    [x],
    lr=0.1,           # try 0.001–0.02; increase if too slow, decrease if jittery
    betas=(0.9, 0.999), # (beta1 for momentum, beta2 for squared grads)
...
plt.scatter([x_min[0].item()], [x_min[1].item()], c='black', s=60, marker='*', label='Optimum')
plt.title(f'Optimization Trajectory (Adam): {n_steps} step(s)')
plt.xlabel('$x_1$')
plt.ylabel('$x_2$')
plt.legend()
plt.grid(True)
...
As a data scientist, you will likely use one of these algorithms when training a machine learning model. In practice, however, you typically won't:

- Prove the convergence of the algorithm for your specific problem  
- Explicitly determine the optimal value of the parameter $\alpha_k$

Nonetheless, it's important to understand the implementation details—especially when things don't go as expected. A solid grasp of the underlying mechanics can help you diagnose issues and make informed adjustments.
...
- we proved linear convergence of the gradient descent method
