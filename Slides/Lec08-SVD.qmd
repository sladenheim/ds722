...
where $U\in\mathbb{C}^{m \times m}$ is unitary, $V\in\mathbb{C}^{n\times n}$ is unitary, and $\Sigma\in\mathbb{R}^{m\times n}$.
...
| Orthonormal bases  | Non-orthonormal bases (in general) |
...
if $\nu=p=\min\{m,n\}$ define $\sigma_{p+1}=0$. Then
...
:::: {.fragment}
Recall from the previous lecture that since the matrix $S$ is symmetric, its eigenvectors can be chosen to be orthonormal &mdash; so the matrix $V$ they form is orthogonal (unitary).
::::
