...
- $A = \{ (B_{1},V), (B_{1},C)\}$, the event the cookie came from bow 1
- $B = \{ (B_{1},V), (B_{2},V)\}$, the event the cookie is vanilla


:::: {.fragment}
What does $\mathbb{P}(A\mid B)$ represent?
::::
...
- $\small \mathbb{P}(\text{Bowl 1})$ which is one bowl out of two, so 1/2.
- $\small \mathbb{P}(\text{Vanilla} \vert \text{Bowl 1})$ which is 30 out of 40 cookies, so 3/4.
- $\small \mathbb{P}(\text{Vanilla})$ which is a little tougher, since it's the combined probability of Vanillas from either bowl. What can we do?
...
$$
\small \begin{align*}
\mathbb{P}(\text{first bowl} \vert \text{Vanilla})\ & = \frac{\mathbb{P}(\text{Vanilla} \vert \text{first bowl})\mathbb{P}(\text{first bowl})}{\mathbb{P}(\text{Vanilla})}\\
...
$$
\small \mathbb{P}(H \,\vert\, D)\; =  \frac{\mathbb{P}(H)\mathbb{P}(D\,\vert\,H) \; }{\mathbb{P}(D)}. 
$$
...
## Monty Hall
Recall from homework our Monty Hall problem with starving tigers.
...
:::: {.fragment}
What are the *hypotheses*? And what is the *likelihood* of the data under each hypothesis? 
::::
