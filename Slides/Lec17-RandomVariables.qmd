...
Flip a fair coin twice and let $X$ be the number of heads. Then $P(X=0) = P(X=2) = 1/4$ and $P(X=1) = 1/2$, the CDF is
...
A function $F$ mapping the real line to $[0, 1]$ is a CDF for some probability $P$ if and only if $F$ satisfies:
...
$X$ is discrete if it takes countably many values $\{x_{1},x_{2},\ldots\}$. We define the *probability mass function* (pmf) for $X$ by $f_{X}(x) = P(X=x)$.
...
$$
\begin{align}
F(b)-F(a) &= P(a< X <b) = P(a\leq X \leq b) \\
...
Let $X$ represent a binary event with probability, $P(X=1) = p$ and $P(X=0)=1-p$ for some $p\in[0,1]$. We say that $X\sim \operatorname{Bernoulli}(p)$ has a Bernoulli distribution and 
...
1. for any set $A\subset \mathbb{R}\times \mathbb{R}$, $P((X,Y)\in A) = \int\int_{A}f(x,y)dxdy$.
...
$$
f_{X}(x) = \sum_{y}P(X=x, Y=y) = \sum_{y}f(x,y).
$$

and the *marginal mass function* for $Y$ is

$$
f_{Y}(y) = \sum_{x}P(X=x, Y=y) = \sum_{x}f(x,y).
$$
...
$$
\small
f_{X\vert Y}(x\vert y) = P(X=x\vert Y=y) = \frac{P(X=x, Y=y)}{P(Y=y)} = \frac{f_{X,Y}(x,y)}{f_{Y}(y)},
$$
...
$$
 P(X\in A\vert Y=y) = \int_{A}f_{X\vert Y}(x\vert y)dx.
$$
