...
The work is dominated by the vector operation in the inner loop $\ell_{j,k:m} = u_{j,k:n} - \ell_{jk}u_{k,k:m}$, a scalar-vector multiplication and one vector subtraction. If $l=m-k+1$, then we have $2l$ flops. 
...
Note that for $L_{2}^{\prime}$ has the same structure of $L_{2}$. Why?
