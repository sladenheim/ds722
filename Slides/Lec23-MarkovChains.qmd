...
$P^{(n)}$ with $P_{ij}^{(n)}  = \mathbb{P}(X_n = i\,\vert\,X_0 = j)$.

Let's compute $P^{(n)}$ for the simple case of $n = 2$:

$P^{(2)}_{ij} = \mathbb{P}(X_2 = i\,\vert\,X_0 = j)$

::: {.notes}
$$
\begin{align}
\sum_k \mathbb{P}(X_2 = i\,\vert\,X_1 = k, X_0 = j)\,\mathbb{P}(X_1 = k\,\vert\,X_0 = j) &= \sum_k \mathbb{P}(X_2 = i\,\vert\,X_1 = k)\,\mathbb{P}(X_1 = k\,\vert\,X_0 = j) \\
&= \sum_{k}P_{ik}P_{kj}
\end{align}
$$
...
We showed that $\mathbb{P}(X_2 = i\,\vert\,X_0 = j) = P^{(2)}_{ij} = P^2_{ij}$, i.e., the two-step transition probabilities are given by $P^{(2)} = P^{2}$.
...
Specifically, $X_0$ is a random variable with distribution $\mathbb{P}(X_0 = i) = \mathbf{x}_{0,i}$.
...
$$\small
\begin{align}
\mathbf{x}_{n+1,i} = \mathbb{P}(X_{n+1} = i) &= 
\sum_j \mathbb{P}(X_{n+1} = i\,\vert\,X_n = j)\,\mathbb{P}(X_n = j)\\
...
:::: {.fragment}
Perron-Frobenius tells us that 1 is always an eigenvalue of $P$. The associated eigenvector $\boldsymbol{\pi}$ is the steady-state probability vector.
::::
...
Check that $\pi_i P_{ji} = \pi_j P_{ij}$ for all $i$, $j$ where $\boldsymbol{\pi} = \begin{bmatrix}\frac{6}{11}\\\frac{3}{11}\\\frac{2}{11}\end{bmatrix}$.
