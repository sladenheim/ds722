...
What does the notation $P(X = x)$ and $P(X\leq x)$ mean?

:::: {.incremental}
1. $P(X = x)$ is the probability that the random variable $X$ takes on the exact value of $x$.
1. $P(X \leq x)$ is the probability that the random variable $X$ is less than or equal to the value $x$.
::::
...
For discrete variables, the *probability mass function* (PMF) of $X$ is defined by $f_{X}(x) = P(X=x)$.
...
$$
F_{X}(x) = P(X\leq x) = \sum_{x_{i}\leq x}f_{X}(x_{i}).
$$
...
1. for any set $A\subset \mathbb{R}\times \mathbb{R}$, $P((X,Y)\in A) = \iint_{A} f(x,y) \, dx \, dy$.
...
$$
f_{X}(x) = \sum_{y}P(X=x, Y=y) = \sum_{y} f_{X,Y}(x,y).
$$

and the *marginal mass function* for $Y$ is

$$
f_{Y}(y) = \sum_{x}P(X=x, Y=y) = \sum_{x} f_{X,Y}(x,y).
$$
...
$$
\begin{align}
F_{Y}(y) &= P(Y\leq y) = P(\log(x)\leq y) \\
&= P(X\leq e^{y}) = F_{X}(e^{y})= 1- e^{-e^{y}} \\
\end{align}
$$
...
Let $X\sim\operatorname{Bernoulli}(p)$, what is $E(X)$?
...
Let $X\sim\operatorname{Uniform}(-1, 3)$, what is $E(X)$?
...
$$
E(X) = \int_{-1}^{3}\frac{1}{4}dx = \frac{1}{4}(3 -(-1))= 1 = (a+b)/2.
$$
...
$$
E(Y) = E(r(X)) = \int_{-\infty}^{\infty} r(x) f_{X}(x) \, dx.
$$
...
$$
\small
E(I_{A}(X)) = \int I_{A}(x)f_{X}(x)dx = \int_{A}f_{X}(x)dx = P(X\in A).
$$
...
E[Y^{2}] &= \int_{0}^{1} y^{2} \, dy = \frac{1}{3}, \\
\therefore \quad E[Z] &= \frac{1}{3} + \frac{1}{3} = \frac{2}{3}.
\end{aligned}
$$
:::
...
**Definition**
Let $X$ be a random variable with mean $\mu$. The variance of $X$, $V(X)=\sigma^{2}=\sigma_{X}^{2}$, is defined by

$$
V(X) = E\big[(X-\mu)^{2}\big] = \int_{-\infty}^{\infty} (x-\mu)^{2} f_{X}(x) \, dx,
$$
...
1. $V(X) = E(X^{2})-\mu^{2}$.
1. If $a$ and $b$ are constants, then $V(aX+b) = a^{2}V(X)$.
...
Let $X\sim\operatorname{Binomial}(n,p)$, what is $V(X)$?

::: {.notes}
$$
E(X_{i}) = p, \quad E(X_{i}^{2}) = (p\cdot 1^2) + (1-p)\cdot o^{2} = p
$$

Then $V(X_{i}) = p - p^{2} = p(1-p)$.

Sum these up and get $np(1-p)$. $V=0$ if $p=0,1$.
:::
...
The denominator has $n-1$ so that $E(S_{n}^{2})= \sigma^{2}$.

---

**Theorem**
Let $X_{1},\ldots, X_{n}$ be IID and let $\mu=E(X_{i})$, $\sigma^{2}=V(X_{i})$, then

$$
E(\bar{X}_{n})=\mu,\quad V(\bar{X}_{n}) = \frac{\sigma^{2}}{n},\quad E(S_{n}^{2})=\sigma^{2}.
$$
...
$$
\small
\operatorname{Cov}(X,Y)=E\left((X-\mu_{X})(Y-\mu_{Y})\right).
$$
...
$$
V\left(\sum_{i=1}^{n}a_{i}X_{i}\right) = \sum_{i=1}^{n}a_{i}^{2}V(X_{i}) + 2 \sum_{i=1}^{n} \sum_{j>i}^{n}\operatorname{Cov}(X_{i},X_{j}).
$$

## Expectation and Variance of Important RVs

| Distribution              | Mean $E[X]$         | Variance $V(X)$           |
|---------------------------|------------------------------|------------------------------------|
| Bernoulli($p$)        | $p$                          | $p(1 - p)$                         |
...
The mean of a random vector $X=[X_{1},\ldots, X_{k}]^{\top}$ is a vector
...
V(X) = \Sigma =
\begin{bmatrix}
V(X_{1}) & \operatorname{Cov}(X_{1}, X_{2}) & \cdots & \operatorname{Cov}(X_{1}, X_{k}) \\
\operatorname{Cov}(X_{2}, X_{1}) & V(X_{2}) & \cdots & \operatorname{Cov}(X_{2}, X_{k}) \\
\vdots & \vdots & \vdots & \vdots \\
\operatorname{Cov}(X_{k}, X_{1})  & \operatorname{Cov}(X_{k}, X_{2}) & \cdots  & V(X_{k}) \\
\end{bmatrix}
$$
...
If $a$ is a vector and $X$ is a random vector with mean $\mu$ and variance $\Sigma$, then $E(a^{\top}X) = a^{\top}\mu$ and $V(a^{\top}X) = a^{\top}\Sigma a$. If $A$ is a matrix then $E(AX) = A\mu$ and $V(AX) = A\Sigma A^{\top}$ 
-->
...
$$
\small
f(\mathbf{x}; \mu, \Sigma) = \frac{1}{(2\pi)^{k/2}\vert \Sigma \vert^{1/2}}\operatorname{exp}\left\{-\frac{1}{2}(\mathbf{x}-\mu)^{\top}\Sigma^{-1}(\mathbf{x}-\mu) \right\},
$$
...
$$
E(E(Y\mid X)) = E(Y)\quad\text{and}\quad E(E(X\mid Y)) = E(X).
$$
...
$$
V(Y\mid X=x) = \int (y-\mu(x))^{2} f_{Y\mid X}(y\mid x) \, dy,
$$
...
In this case, what is $E(X\mid Q=q)$ and $V(X\mid Q=q)$?

:::: {.fragment}
$$
E(X\mid Q=q) = nq, \quad V(X\mid Q=q)=nq(1-q).
$$
::::
...
We can now use what we learned in class to compute $E(X)$ and $V(X)$.

---

::: {.notes}
$$ E(X) =  E(E(X\mid Q)) = nE(Q) = n/2$$ 

$$
V(X) = E(V(X\mid Q)) + V(E(X\mid Q)) 
$$

$$
E(V(X\mid Q)) = E(nQ(1-Q)) = n\int_{0}^{1} q(1-q) \, dq = n/6
$$

$$
V(E(X\mid Q)) = V(nQ) = n^{2}V(Q) = n^{2}\int_{0}^{1}\big(q-\tfrac{1}{2}\big)^{2} \, dq = n^{2}/12.
$$
:::
...
The $k$th moment of $X$ is defined to be $E(X^{k})$ assuming that $E(\lvert X\rvert^{k})<\infty$.
...
::: {.notes}
Skewness measures the asymmetry of a distribution about its mean. It is defined as the standardized third central moment:

$$
\text{skewness} = \gamma_1 = \frac{E\big[(X-\mu)^3\big]}{\sigma^3}.
$$

Interpretation: $\gamma_1>0$ indicates right-skew (long right tail), $\gamma_1<0$ indicates left-skew (long left tail), and $\gamma_1\approx 0$ suggests approximate symmetry.

Kurtosis measures the "tailedness" or peakiness of a distribution and is the standardized fourth central moment:

$$
\text{kurtosis} = \beta_2 = \frac{E\big[(X-\mu)^4\big]}{\sigma^4}.
$$

Excess kurtosis $=\beta_2-3$ is commonly used so that the normal distribution has excess kurtosis 0. Positive excess kurtosis (leptokurtic) implies heavier tails / more extreme outliers; negative excess kurtosis (platykurtic) implies lighter tails / fewer outliers; zero (mesokurtic) matches the normal.
:::
...
$$
\psi_{X}(t) = E(e^{tX}) = \int e^{tx}f_{X}(x)dx.
$$
...
$$
\small
\psi^{\prime}(0) = \frac{d}{dt}E(e^{tX})\bigg\vert_{0} = E\left(\frac{d}{dt}e^{tX}\right)\bigg\vert_{0}  = E(Xe^{tX})\bigg\vert_{0}  = E(X).
$$

We can similarly conclude that $\psi^{(k)}(0) = E(X^{k})$.
