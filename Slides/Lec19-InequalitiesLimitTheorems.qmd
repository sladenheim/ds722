...
Let $X$ be a nonnegative random variable and suppose that $E(X)<\infty$. For any $t>0$,
...
&\geq \int_{t}^{\infty}xf(x)dx\geq t\int_{t}^{\infty}f(x)dx = tP(X>t).
\end{align}
$$
:::
...
::: {.notes}
If the average is $E(X) = 50$, an upper bound on the proportion of students who score at least 100 should be 50\%, right? If more than 50\% of students scored 100 (or higher), the average would already be at least 50, since all scores must be nonnegative $\geq 0$. Mathematically, we just argued that:

$$
P(X \geq 100) \leq \frac{E[X]}{100} = \frac{50}{100} = \frac{1}{2}
$$
...
$$
P(X \geq 100) \leq \frac{E(X)}{100} = \frac{25}{100} = \frac{1}{4}
$$
:::
...
$$
P(\vert X-\mu\vert \geq t)\leq \frac{\sigma^{2}}{t^{2}}\quad\text{and}\quad P(\vert Z\vert \geq k)\leq \frac{1}{k^{2}},
$$

where $Z=\frac{X-\mu}{\sigma}$. In particular $P(\vert Z\vert > 2)\leq 1/4$ and $P(\vert Z\vert > 3)\leq 1/9$.

::: {.notes}
Use Markov's inequality.

$$
P(\vert X-\mu\vert \geq t) = P(\vert X-\mu\vert^{2} \geq t^{2}) \leq \frac{E\left((X-\mu)^{2}\right)}{t^{2}} = \frac{\sigma^{2}}{t^{2}}
$$
The second part follows by setting $t=k\sigma$.

The set of random values where $\vert X-\mu\vert \geq t$ is the same as $\vert X-\mu\vert^{2} \geq t^{2}$, $\vert a \vert \geq t$ iff $\vert a\vert^{2}\geq t$.
:::
...
Suppose we use a neural net to make a prediction on $n$ new test cases. Let $X_{i}=1$ if the NN is wrong and $X_{i}=0$ if the NN is correct. Can we approximate the unknown error rate $p$?
...
$P(\vert\bar{X}_{n} - p\vert > \varepsilon) \leq$


::: {.notes}
$$
\smallP(\vert\bar{X}_{n} - p\vert > \varepsilon) \leq V(\bar{X}_{n}){\varepsilon^{2}} = \frac{np(1-p)}{n^{2}\varepsilon^{2}}\leq \frac{1}{4n\varepsilon^{2}}
$$
:::
...
**Theorem**
Let $X_{1},\ldots,X_{n}$ be independent observations such that $E(X_{i})=0$ and $a_{i}\leq X_{i}\leq b_{i}$. Let $\varepsilon>0$. Then, for any $t>0$,

$$
P\left(\sum_{i=1}^{n}X_{i}\geq \varepsilon\right) \leq e^{-t\varepsilon}\prod_{i=1}^{n}e^{t^{2}(b_{i}-a_{i})^{2}/8}.
$$
...
$$
P(\vert \bar{X}_{n}-p\vert > \varepsilon) \leq 2\varepsilon e^{-2ne^{2}},
$$
...
Recall that the Chebyshev' bound  $P(\vert \bar{X}_{n}-p\vert > \varepsilon)$ when $n=100$ and $\varepsilon=0.2$ was 0.0625.

Using the Hoeffding bound, we obtain

$$
P(\vert \bar{X}_{n}-p\vert > 0.2) \leq 2e^{-2(100)/(0.2)^{2}} = 0.00067.
$$ -->
...
$$
P(\vert Z\vert > t)\leq \sqrt{\frac{2}{\pi}}\frac{e^{-t^{2}}}{t}.
$$
...
:::: {.fragment}
You can prove this by expanding out $E((X-\alpha Y)^{2})$ and setting $\alpha = \frac{E(XY)^{2}}{E(Y^{2})}$
::::
...
::: {.notes}
Let $L(x) = a+bx$ be the line, tangent to $g(x)$ at the point $E(X)$. Since $G$ is convex, it lies above the line $L(x)$, so

$$
E(g(X))\geqE(L(x)) = E(a+bX) = a + bE(X) = L(\mathbb{E(X)}) = g(E(X)).
$$
::: -->
...
$$
P(\vert X_{n}-X\vert >\varepsilon) \rightarrow 0,
$$
...
$$
\small
Z_{n} = \frac{\bar{X}_{n}-\mu}{\sqrt{V(\bar{X}_{n})}} = \frac{\sqrt{n}(\bar{X}_{n}-\mu)}{\sigma} \leadsto Z,
$$

where $Z\sim\mathcal{N}(0,1)$. In other words

$$
\lim_{n\rightarrow\infty}P(Z_{n}\leq z) = \Phi(z) = \int_{-\infty}^{z}\frac{1}{2\pi}e^{-x^{2}/2}dx.
$$
...
We want to approximate $P(\bar{X}_{n} < 5.5)$ using the CLT.

What do we need?

:::: {.incremental}
1. The mean $\mu = E(X_{i})= \lambda = 5$
1. The variance $\sigma^{2} = V(X_{i}) = \lambda = 5$.
::::
...
::: {.notes}
$$
P(\bar{X}_{n} < 5.5) = P\left(\frac{\sqrt{n}(\bar{X}_{n}-\mu)}{\sigma} < \frac{\sqrt{n}(5.5-\mu)}{\sigma} \right) \approx P\left(Z < 2.5 \right) = 0.9938
$$
:::
