...
A set of vectors $S = \{q_1, q_2, \ldots, q_n \}$ is orthogonal if every pair of vectors is orthogonal, i.e.,
...
In order for $\hat{Q}$ to span the successive column spaces of $A$, we must have
...
If we divide $v_{j}$ by $\Vert v_{j}\Vert_2$, then we have an orthonormal $q_{j}$. If we have repeated this process $j$ times, then $q_{j}\in\langle a_{1},\ldots, a_{j} \rangle$ and by construction is orthogonal to $q_{1},\ldots, q_{j-1}$. 
...
1. for $j=1,\ldots, n$
1. $\quad v_{j} = a_{j}$
1. $\quad$ for $i=1,\ldots, j-1$
1. $\quad\quad r_{ij} = q_{i}^{\top}a_{j}$ 
1. $\quad\quad v_{j} = v_{j} - r_{ij}q_{i}$
1. $\quad r_{jj} = \Vert v_{j}\Vert_2$ 
1. $\quad q_{j} = v_{j}/r_{jj}$ 
...
### Step 4 ($j=3$), after the inner loop
...
The matrix $R$ is created by appending $m-n$ rows of zeros to $\hat{R}$.
...
The CGS method is not numerically stable. However, we can modify this method to produce a more stable algorithm.
...
1. $\quad$ for $j=i+1,\ldots, n$
1. $\quad\quad r_{ij} = q_{i}^{\top}v_{j}$ 
1. $\quad\quad v_{j} = v_{j} - r_{ij}q_{i}$
...
In the code cell below I create a random matrix of dimension $500 \times 500$ and compute the QR factorizations using the CGS and MGS methods.

The Frobenius norm $\Vert I - Q^{\top}Q\Vert_{F}$ is printed to the screen.  
...
def classical_gram_schmidt(A):
    ...
    for i in range(n_cols):
        # Start with a copy of the original column, so that A is left unchanged
        q = A[:, i].copy()
        for j in range(i):
            # Subtract the projection of the ORIGINAL column onto each previous q
...
At the first step in the outer loop (Step 3), the MGS algorithm operates on the whole $m\times n$ matrix.
...
:::: {.fragment}
A right triangular prism with a volume of $mn^{2}/2$.
::::
...
Step $i$ of the MGS algorithm subtracts $r_{ij}/r_{ii}$ times column $i$ of the current $A$ from columns $>i$ and replaces column $i$ by $1/r_{ii}$ times itself. These operations define the upper-triangular matrices $R_{i}$. 
...
<!-- For MGS the loss of orthogonality grows like $\kappa(A)$, whereas for CGS it grows like $\kappa(A)^{2}$. -->
