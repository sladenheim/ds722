...
$P(\Theta = \theta \mid X = x) =$

::: {.notes}
$$
\begin{align}
P(\Theta = \theta \mid X = x) &= \frac{P(X=x,\Theta=\theta)}{P(X=x)} \\
&= \frac{P(X=x\mid \Theta=\theta) P(\Theta=\theta)}{\sum_{\theta} P(X=x\mid \Theta=\theta) P(\Theta=\theta)}
\end{align}
$$
:::
...
For continuous variables we replace $P$ with density functions so that
...
## Biased Coins
Consider an experiment with a possibly biased coin. We will consider $\Theta = P(H)$. Suppose that before conducting the experiment we believe that all values of $\theta$ are equally likely.
...
What is the most likely value for $\Theta=P(H)$?
...
For example, suppose we want to determine an interval $[a, b]$ in which we are 90% confident that the parameter $\theta$ lies, i.e., $P(\theta\in[a,b]\mid x_{1},\ldots, x_{n}) = 0.90$
...
$$
\small
P(\theta\in[a,b]\mid x_{1},\ldots, x_{n}) = \int_{a}^{b}f(\theta\mid x_{1},\ldots, x_{n})d\theta = 0.90,
$$
