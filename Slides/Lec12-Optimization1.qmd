...
Suppose we are training a parameterized model $y=h_{\theta}(x)$. We have labeled training data $\left\{(x_{i}, y_{i})\right\}_{i=1}^{m}$. 

We have the predicted outputs $\hat{y}_{i} = h_{\theta}(x_{i})$ and a loss function $\mathcal{L}(\hat{y}_{i}, y_{i})$

In supervised learning we aim to minimize the average of all the loss functions, i.e., we solve

$$
\small
\min_{\theta} \frac{1}{m}\sum_{i=1}^{m}\mathcal{L}(\hat{y}_{i}, y_{i}) = \min_{\theta}\frac{1}{m}\sum_{i=1}^{m}\mathcal{L}(h_{\theta}(x_{i}), y_{i}).
$$
...
Necessary conditions are derived by assuming that $x^{*}$ is a local minimizer and proving facts about $\nabla f(x^{*})$ and $\nabla^{2} f(x^{*})$.
...
# Sequence definition
# Note: use float (not int) exponentiation -- 2**(2**k) overflows int64
# for k >= 6, which silently wraps around and corrupts the values.
k = np.arange(0, 10, dtype=float)
x_k = 1 / 2.0**(2.0**k)
...
k_quadratic = np.arange(0, 10, dtype=float)
xk_quadratic = 1 / 2.0**(2.0**k_quadratic)
...
axs[0].plot(k_quadratic, xk_quadratic, '^-', label='Quadratic: $1/2^{2^k}$')
...
axs[1].semilogy(k_quadratic, xk_quadratic, '^-', label='Quadratic: $1/2^{2^k}$')
