...
In this lecture we start to cover sequences of dependent random variables.
...
1. Let $\mathcal{X}=\{\text{sunny}, \text{rainy}\}$. Then we can consider $X_{t}$ being the random variable of the weather on day $t$. 
1. Stock prices.
...
$$
P(X_{n}=x\mid X_{0},\ldots, X_{n-1}) = P(X_{n}=x\mid X_{n-1}).
$$
...
$$
\small
P(X_{n+1} = i \,\vert\, X_{n} = j, X_{n-1} = x_{n-1}, \dots, X_0 = x_0) =
$$

::: {.notes}
$P(X_{n+1} = i \,\vert\, X_{n} = j)$
:::

## Transition Matrix

The transition matrix of the Markov chain is the matrix $T$ with 
$$
T_{ij}  = P(X_{n+1} = i \,\vert\, X_{n} = j).
$$

__Remark__: $\small T_{ij}$ is the **transition probability** from state $\small j$ to state $\small i$.<br>

$\small T$ has the following properties:

1. $\small T_{ij} \geq 0 \quad \forall i, j$
2. $\small \sum_i T_{ij} = 1 \quad \forall j$

::: {.notes}
The second property states that each column of $\small T$ sums to 1.  A matrix with this property is called a __stochastic__ matrix (or more specifically, a __column-stochastic matrix__)
:::
...
![](figures/Sunny_Rainy2.png){fig-align="center" width="80px"}
...
$T^{(n)}$ with $T_{ij}^{(n)}  = P(X_n = i\,\vert\,X_0 = j)$.

Let's compute $T^{(n)}$ for the simple case of $n = 2$:

$T^{(2)}_{ij} = P(X_2 = i\,\vert\,X_0 = j)$
...
We showed that $P(X_2 = i\,\vert\,X_0 = j) = T^{(2)}_{ij} = T^2_{ij}$, i.e., the two-step transition probabilities are given by $T^{(2)} = T^{2}$.
...
We can confirm that by computing $T^2$:
...
The __Forward Kolmogorov Equation__ tells us that taking one step forward in time corresponds to multiplying $\small T$ times $\small \mathbf{x}_n$. This follows from the law of total probability:

$$\small
\begin{align}
\mathbf{x}_{n+1,i} = P(X_{n+1} = i) &= 
\sum_j P(X_{n+1} = i\,\vert\,X_n = j)\,P(X_n = j)\\
&= \sum_j T_{ij} \mathbf{x}_{n,j} \\
&= (T\mathbf{x}_n)_{i}.
\end{align}
$$
 
This means that $\mathbf{x}_n = T^n \mathbf{x}_0$.

::: {.notes}
That is, if we know the initial probability distribution at time 0, we can find the distribution at any later time using powers of the matrix $T$.
:::
...
Suppose you live in a country with three political parties $A$, $B$, and $C$.
 We use $A_k, B_k,$ and $C_k$ to denote the percentage of voters voting for that party in election $k$.
...
:::
::: {.column width="60%"}
Suppose that initially 40% of citizens vote for party $A$,
30% vote for party $B$, and 30% vote for party $C.$
...
$$
T = \begin{bmatrix} 0.6 & 0 & 0.2\\ 0.4 & 0.6 & 0.2 \\ 0 & 0.4 & 0.6\end{bmatrix}.
$$
:::
...
$$
\mathbf{x}_1 = T\mathbf{x}_0 = \begin{bmatrix} 0.6 & 0 & 0.2\\ 0.4 & 0.6 & 0.2 \\ 0 & 0.4 & 0.6\end{bmatrix}\begin{bmatrix} 0.4\\ 0.3 \\ 0.3\end{bmatrix} = \begin{bmatrix} 0.3\\ 0.4 \\ 0.3\end{bmatrix}.
$$

$$
\mathbf{x}_{2} = T\mathbf{x}_{1} = \begin{bmatrix} 0.6 & 0 & 0.2\\ 0.4 & 0.6 & 0.2 \\ 0 & 0.4 & 0.6\end{bmatrix}
...
$$
\small T = \begin{bmatrix}0.8 & 0.4\\0.2 & 0.6\end{bmatrix}.
$$

What happens to the weather in 100 or 1,000 days?

Assume that day 0 is sunny, and let the chain run for a while.

::: {.notes}


We can calculate the probability of a sunny day in 2 days or in 10 days by using the powers of $T$. 
:::
...
# T is one-step transition matrix
...
This model is converging to the vector: $\small{\begin{bmatrix} 2/3 \\ 1/3 \end{bmatrix}}$.
...
A vector $\mathbf{x}$ for which $T \mathbf{x} = \mathbf{x}$ is called a steady state of the Markov chain corresponding to $T$.
...
:::: {.fragment}
However, if the chain has a finite set of states, then it can be represented by a column stochastic matrix $T$.

Did we see any other examples of column stochastic matrices this semester? What did we learn about their eigenvalues?
::::

:::: {.fragment}
Perron-Frobenius tells us that 1 is always an eigenvalue of $T$. The associated eigenvector $\boldsymbol{\pi}$ is the steady-state probability vector.
::::
...
Solve the linear system $(T-I)\mathbf{x}=0$ or use numpy to find the eigenvalue/eigenvector pairs.
:::
...
$$ 
\pi_i T_{ji} = \pi_j T_{ij}\;\;\; \forall i, j. 
$$
...
Check that $\pi_i T_{ji} = \pi_j T_{ij}$ for all $i$, $j$ where $\boldsymbol{\pi} = \begin{bmatrix}\frac{6}{11}\\\frac{3}{11}\\\frac{2}{11}\end{bmatrix}$.

::: {.notes}
To check if it is in detailed balance, we need to confirm that $\pi_i T_{ji} = \pi_j T_{ij}$ for all $i$, $j$ for
$\small{T = \begin{bmatrix}0.8&0.2&0.3\\0.1&0.6&0.3\\0.1&0.2&0.4\end{bmatrix}}$ and $\small{\boldsymbol{\pi} = \begin{bmatrix}\frac{6}{11}\\\frac{3}{11}\\\frac{2}{11}\end{bmatrix}}.$

We do not need to check cases where $i=j.$ For example, for $i=1$ and $j=1,$ the detailed balance equation is $\small{\pi_1 T_{1,1} = \pi_1 T_{1,1}}$ is always true.
...
A Markov chain is a *regular* Markov chain if some matrix power of the transition matrix $T^k$ contains only strictly positive entries. 
