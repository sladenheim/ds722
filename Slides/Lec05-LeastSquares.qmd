...
ax.plot([b[0], proj_b[0]], [b[1], proj_b[1]], [b[2], proj_b[2]], 'k--', label=r'$r = b - \hat b$')
...
# Annotate points
plt.text(original_point[0]+0.1, original_point[1], r'$b$', fontsize=12)
plt.text(original_point[0]-0.25, original_point[1]+0.5, r'$r = b - Ax$', fontsize=12)
plt.text(projection_point[0]+0.1, projection_point[1], r'$y = Ax = Pb$', fontsize=12)
plt.text(-0.25, 0.5, r'$\operatorname{range}(A)$', fontsize=12)
...
$$
where $P=A(A^{\top}A)^{-1}A^{\top}\in\mathbb{R}^{m\times m}$ is the orthogonal projector onto $\operatorname{range}(A)$ &mdash; the same projector derived from an arbitrary basis in Lecture 3.
...
> Solving this system by computing $(A^{\top}A)^{-1}$ is not numerically stable!
