...
If $a$ is a vector and $X$ is a random vector with mean $\mu$ and variance $\Sigma$, then $\mathbb{E}(a^{\top}X) = a^{\top}\mu$ and $\mathbb{V}(a^{\top}X) = a^{\top}\Sigma a$. If $A$ is a matrix then $\mathbb{E}(AX) = A\mu$ and $\mathbb{V}(AX) = A\Sigma A^{\top}$ 
-->
...
$$
\text{skewness} = \gamma_1 = \frac{\mathbb{E}\big[(X-\mu)^3\big]}{\sigma^3}.
$$
...
$$
\text{kurtosis} = \beta_2 = \frac{\mathbb{E}\big[(X-\mu)^4\big]}{\sigma^4}.
$$
